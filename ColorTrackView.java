...
public class ColorTrackView extends View {
	...
	private void drawText_h(Canvas canvas, int color, int startX, int endX) {
		...
		canvas.clipRect(startX, 0, endX, getMeasuredHeight());// left, top,
																// right, bottom
		//(descent-ascent)
		//mTextStartX,getMeasuredHeight() / 2 - (mPaint.descent()-mPaint.ascent)()/2-mPaint.ascent() : 得到BaseLine的位置
		//mTextStartX,getMeasuredHeight() / 2-mPaint.descent()/2 + mPaint.ascent()/2 - mPaint.ascent();
		//mTextStartX,getMeasuredHeight() / 2-(mPaint.descent()+mPaint.ascent())/2;
	...
	private void drawText_v(Canvas canvas, int color, int startY, int endY) {
		...
		canvas.save(Canvas.CLIP_SAVE_FLAG);
		//将当期画布裁剪为一个矩形,以后画的图像仅仅只能画在这块矩形区域中, 超出部分完全看不见
